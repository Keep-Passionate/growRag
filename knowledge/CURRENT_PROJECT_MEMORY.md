...
更新日期：2026-09-27（续轮：S2G–Qwen v4八题配对验收完成）
状态：v4作者控制流程API迁移小测8/8完整完成；BASE与S2G的EM均6/8，F1分别.83333/.89583，差异仅来自1题部分改善。42次真实API返回固定qwen3.7-flash-2026-07-15；非原LoRA或全维基复现。容量变更通过1654项测试后以干净提交beaba3e启动；当前新增数据工具后全仓1672项测试通过。累计834请求，已知估算0.1352172元、保守预留1.0170678元、历史1费用未知，授权50元。训练0、建卡0、租卡0；旧6PRE卡/24检查题不变。新32题＋10021共享文档已离线制备，尚未运行其模型问答。
权威性：当前入口；旧路线 A 决策保留为历史记录，但不再代表当前主线

## 2026-09-27续轮：工程门通过，回到机会与适用条件研究

- 数据制备实际完成：`data/hotpotqa/shared_sep27_v1/`，32题／1024来源／10021文档／10020标题；1个同标题异文保留。manifest SHA `2dc35c7baa470b9714b063c70221aa70a42654a7d3afe8894e5a2dfe67889ab2`。报告`docs/experiments/2026-09-27_共享32题数据冻结记录.md`。18新合成测试、全部1672测试通过；核心src/tests/scripts Ruff与174文件format通过，旧Zotero脚本另有未修改的格式问题。
- 新模块`shared_hotpot_dev.py`仅离线、默认只读，显式制备且拒覆盖。4份JSONL及72条gold句文档版本引用复核通过。只有runtime_questions/corpus可给方法看；corpus_sources也是审计私有，不能借此恢复每题小候选池。下一步共享runner仍待接通，不能继续使用旧按title合并的索引；本轮新32模型请求为0。

- 最新结果：`knowledge/experiments/2026-09-27_S2G_v4八题验收与研究下一步.md`；阶段门槛：`docs/experiments/2026-09-27_研究主线与阶段门槛.md`。下面较早v1–v3的停止状态是历史快照，不再表示当前仍未跑通。
- 用户要求继续推进并时刻明确创新。本轮冻结v4计划后，只运行一次新容量协议，旧三版不覆盖不混算。Judge/Extractor/Answer上限768/128/256；schema仅前两角色；原prompt、gap、作者main_batch不改。真实42调用均HTTP200、finish_reason=stop、live_api、无重试；request SHA与用量逐项复核。
- 主线明确：固定底座/候选数/执行预算，先检验历史是否提供强FRESH未覆盖的机会，再检验适用条件的最少有效表示。不是“S2G＋ReFormeR路由”拼装创新，不把净收益公式声称首创。缺口改写和抽取诊断用于建立公平强FRESH，不自动转成另一个项目。
- v4两路EM都6/8、修复0/损害0；只有005 F1从0→.5。005真的两轮检索，BASE原始支持召回.5、S2G最终1，但答案只singer、gold含songwriter，不能称完整救回。S2G各题最终原始支持全覆盖不等于保留句全覆盖，更不等于Judge语义正确。小候选池、8题反复调试及不等预算均限制结论。
- 下一步G1：官方train中新32题（16 bridge／16 comparison），排除29份历史元数据所覆盖的546个保留／曝光ID；seed20260927、题ID哈希选择。32来源全context＋992独立哈希背景全context构成共享封闭文档池；不是fullwiki，不根据gold筛文档，不读官方dev/test。32题及后代永久不得回流为后续经验／路由训练；背景本轮只作语料，不自动成为记忆来源。题ID不重合不保证文档／实体独立。
- 文档ID按标题＋完整原始句序列哈希；同标题异文不能覆盖合并。runtime题目、gold评分文件和语料分开，后续runner仅在评分阶段取gold，禁止将其用于gap、选动作或生成。此数据导出不产生API费用。
- G2同状态同预算query对照→G3同候选数历史/FRESH机会矩阵→G4条件字段消融与等长控制→G5冻结保留集评测。若无历史独立机会，不强行训练复杂controller；任何转题先与用户讨论。每輪执行前状态选择与首次检索前纯q-only分开命名。
- v4本批估算.0070194／保守预留.0461584，已纳入834次全项目账；下次对账必须在HISTORY_ROOTS之外包含debug8_v1、json8_v2、schema8_v3、capacity8_v4四份final_budget。已知费用不是账单，未知1次继续保守预留。
- 当前API＋CPU足够，不需要SSH／4090；不会自动租卡或训练原Judge。作者无LICENSE快照留ignored external，不公开分发。用户暂不需决定最终卡片字段或学习率；下一次有意义的选择依据新32题的失败分布提出。

## 2026-09-27：真实S2G迁移与研究切口

- 先读 `knowledge/experiments/2026-09-27_S2G_Qwen迁移真实结果与决策.md`；配置与实时查看 `docs/reproduction/2026-09-27_S2G_API迁移配置与实时查看.md`；研究排重 `knowledge/literature/2026-09-26_S2G改进切口核验.md`。Obsidian新增 `D:/notes/GrowRAG/专题精读/2026-09-27/`，旧批注不覆盖。
- 用户回复“按照你的理解来……由你决定”，委托处理API兼容。v1纯提示第6题Extractor64截断；v2加JSON object，第3题sufficient误写specific；v3严格Schema解决已有字段约束，第6题Extractor已通过，但Judge长description达到256截断。严格Schema不保证语义正确或无限容量。旧失败输出、claim、费用全部保留，不猜修字段，不把协议失败评分EM0。
- v3运行代码9f86493；作者桥接de52c9f、JSON阶段9b4bd8e。`s2g_author_api.py`加载4个SHA锁定作者文件的审阅函数，实际运行main_batch，仅替换推理后端；`run_s2g_author_pilot.py`分角色输出格式/上限、共享经费账本、实时日志。作者无LICENSE源码不公开分发。当前有格式严格旁路、文档小池BM25、regex分句等明确差异，不冒称原论文模型复现。
- v3只在Judge/Extractor加strict JSON Schema；Answer仍原文本；上限仍作者256/64/128。输出schema注册表v2，原四schema指纹不变。1645测试、Ruff/172文件格式检查通过。参数改变会另立协议，不覆盖旧运行。
- v3五完整题BASE/S2G都是EM0.8、F1.9333，全部只检索一次。第6题失败、第7/8题未开始；小池和非随机完成子集不足以排名。003等编号见报告，不根据小样本选“最好版本”。002的Lincoln身份文档已检到却未保留，S2G答母公司Ford；不是已证实的query失败，恢复身份句还未做因果对照。
- 逐条对账新增69请求/已知.0116214；累积792/.1281978/.9709094/未知1。下次核账在原HISTORY_ROOTS之外必须加入三个根：2026-09-27_s2g_author_api_debug8_v1、2026-09-27_s2g_author_api_json8_v2、2026-09-27_s2g_author_api_schema8_v3，各自final_budget.json。顶层transport_failure在v1/v3实际是HTTP200+length，报告明确为截断，不回写原审计。
- 本轮明确停止第四次付费重跑。下一批建议单列Qwen容量Judge768/Extractor128/Answer256，先少量工程状态，后跑8题；只是待冻结配置，尚未编码或验证。普通API阶段不需要4090，输出上限问题不是本地显存问题。需要加载/训练本地Judge再问SSH，租卡不自动包含在API50元授权内。
- 研究判断：ReFormeR+S2G+二选一路由不足以单独构成贡献；ExpWeaver已有可选经验、ProactAgent已有配对经验收益。优先诊断后选择：A约束/实体绑定保真的gap→query，B检索缺失vs压缩丢失的操作区分；历史最少条件表示后置。S2G原query构造保留原问题，不能声称它简单删除限制词。所有切口尚是假设，不保证新颖性或SCI分区。
- 官方validation/test仍完全保留，不用于建库/调prompt。32–64新train诊断应等协议稳定且扩展检索范围后再开始；严格同状态/同预算对照，再训练小选择器。当前未新增S2G改进机制、历史训练或有益复用结论。

## 2026-09-26：先迁移作者S2G，再做历史与FRESH路由

- 用户新决定优先：不先跑我们独立实现的S2G式适配；先审阅、固定并借用作者实现，基线跑通后仅做小增量。报告：`docs/reproduction/2026-09-26_S2G作者代码迁移与历史路由决策.md`；Obsidian同名笔记在`D:/notes/GrowRAG/专题精读/2026-09-26/`。
- 作者仓库`nianaaa/S2G-RAG`固定提交`5d842a67a0a99a7b545bbad0dc402ceaae0e5eff`，下载在ignored `external/s2g_author_snapshot/nianaaa-S2G-RAG-5d842a6/`。本轮未找到LICENSE，不把作者源码/修改版提交公开仓库。`s2g_upstream_check.py`只执行4个SHA锁定且已审阅的纯函数；`runs/2026-09-26_s2g_author_audit/preflight.json`记录24文件语法/6项行为检查、依赖缺口；这不是完整推理成功。
- S2G训练的是3B LoRA充分性/缺口Judge，不是跨题记忆，也不是全部回答模型。论文2804状态、2523/281、3epochs；作者训练脚本按快照随机切分。若重训，我们按question ID分组。当前未核验到官方LoRA、清洗监督JSONL；Releases为0，公开求权重issue仍开放，不等于证明全互联网没有权重。
- `--gpt`只替换提取/回答，仍加载Judge底模与LoRA；原路径还有API变量名不一致和固定OpenAI端点，不能直接填Qwen密钥就运行。待用户选：取得作者权重 / 作者代码API Judge迁移版 / 自行重训。建议先固定Qwen的明确标注迁移版，不冒称原模型复现；本轮未实施该迁移或付费调用。
- 测试隔离定案：Hotpot官方validation/test、2Wiki dev/test、Trivia validation/test全部保留。只在官方train内划分互斥memory-build/router-fit/tuning-dev；卡片后代继承来源ID，正式评测不更新记忆、不反复回调参数。作者叫dev的数据也不能用于我们建卡。语料文档不等于评测问题/答案；BM25加载BEIR的split=test后只用corpus不能解释为可使用test监督。
- 新`check_memory_source` helper拒绝非train、白名单外ID、重复ID与保留题交集，已有合成测试；**尚未接管所有旧建卡入口，不能称全局防火墙**。作者新建卡入口必须接入后才允许执行。本轮没有加载官方保留题或生成真实12题新清单。
- 重要排重：ProactAgent / Ask Only When Needed (arXiv2604.20572)已有同一prefix下有/无经验检索配对，按效果和步数差奖励训练策略；Zhao ExpWeaver (2605.07164)已有动态选择经验并含Hotpot；RRM已有本题修复与跨题经验互补。不能主张“历史+FRESH路由”或“净收益奖励”本身首创。这三篇本轮仅确认预印本状态，不擅自升格。
- 历史参照：ReFormeR公开10模式可做静态规则探针，不等于训练好模型或真实动态记忆；已发现原样示例国家/意图漂移，保留原版与独立修正版边界，来源重合仍需审计。动态流程优先参考有官方许可代码的ReasoningBank（ICLR2026），效用更新参考MemRL，query-only/适用条件参考RRM；没有核验到可直接插入本任务的预制Hotpot经验库。
- 最小插入点：作者Judge→gap→可关闭查询适配器(FRESH作者动作/REUSE历史动作)→同一检索/原句提取→Judge。后续轮有证据，这是POST状态路由；严格q-only首检前路由独立作对照，不混权限。冻结其他模块，先验证是否有互补机会，再训练选择器，不先上RL。
- 下一阶段建议1–2合成接口＋8道train开发题；作者T=4/top_docs=6，论文gap K=1与代码Hotpot默认3分别命名。建议子限5元/100请求、累计50元、不自动重试，未执行。完整语料结果与小候选池接口测试分开；数据ID、每轮原始输出、模型版本、证据与费用必须留痕。
- 同轮前半部分的Dual guarded_v2（默认关闭）、独立S2G pointer适配、新12题manifest/runner仅作为隔离实验工具保留。`run_fresh_continuation.LIVE_PROTOCOL_ENABLED=False`，真实入口在读数据/密钥前拒绝；旧冻结协议已标注被新决定取代。没有更改旧基线默认prompt、旧结果或旧库。
- 验证：1573项离线测试通过，ruff检查和160个Python文件格式检查通过；本轮额外付费0。尚无S2G端到端分数，不把09-24 DualRAG 6/8结果混进S2G。作者GPU/索引/权重依赖未安装，本机8GB显存，当前不租卡。

## 2026-09-24续轮：精读、基线适配、选题收窄

- 双语重点精读在docs/reading/，包括S2G与DualRAG的正式PDF Abstract/Conclusion/Limitations及部分Intro，DualRAG另含Future work；两篇没有独立Discussion标题，以分析章节替代。许可CC BY4.0、非官方翻译已标明。Obsidian新目录D:/notes/GrowRAG/专题精读/2026-09-24/，旧笔记不覆盖。
- S2G审计更正：主方法3B LoRA，清洗2804逐轮快照、2523训练/281验证、3epochs；不是全Hotpot训练。附录K=1，仓库默认最多3。Figure4已有仅替换Judge的GPT-4o-mini教师变体，因此可以先不训练验证，但全链Qwen适配不能叫该原始变体。快照随机划分存在同题泄漏风险，我们后续按question ID分组。
- S2G Figure2真值只是检索到支持文档标题的代理标签；全样本6.44%不是通常FPR，重算约11.77%，充分预测精确率约68.02%也不等于语义充分率。下一阶段必须分文档到达/句子保留/判断/回答，不能把supported当引用ID合法。
- 新记忆排重：ReasoningBank ICLR2026，ProcMEM ICML2026（arXiv v3改名Skill-Pro），RRM/MemRL/MemHarness本轮仅确认预印本。条件、拒用、价值更新、程序记忆均已有；保留用户三问题，重点固定动作/预算下PRE最少条件与相对强FRESH收益，不宣称首创净收益奖励。
- 新代码dualrag_adapter.py和run_dualrag_pilot.py：独立prompt/实现，无作者仓库代码复制；BASE原句Reader与Dual摘要Reader不同，最多2轮/每轮2实体每实体2query，句子BM25，没有BGE rerank。105新测试，全仓1421测试通过；实际代码快照48f2fdb322624d31256bebb42f62493a4d2208a1e8f372b254f306c05d60a23b，所有53模型返回固定qwen3.7-flash-2026-07-15。
- runs/2026-09-24_dualrag_debug8_v1已完整执行，不能删除claim重跑。报告knowledge/experiments/2026-09-24_DualRAG八题真实运行与逐题审查.md。EM救回5/损害0，但F1有1题退步；8题已暴露、预算不同，不能得出正式优势或记忆有效性。本轮无训练、无经验库更新、无服务器租赁。
- 关键诊断：002/005零证据时Reasoner凭模型知识停止，终答弃答；000跨轮省份事实未完整继承引用；004直接县归属句已检到但没送进摘要，不能把它当query失败。下轮先冻结新协议修空证据/未绑定query不变量，再接S2G原句指针与同预算新开发对照。
- 本批估算.009309元/预留.0726672元；全部历史重审通过，723请求/已知.1165764/预留.9119286/历史1未知。下次对账必须在之前4个extra根外再加2026-09-24_dualrag_debug8_v1/final_budget.json。代码/提示已快照，API真实内容/密钥/数据均不提交公开仓库。
- 本机RTX4060 Laptop8GB＋约16GB主存。当前API诊断无需租卡；单独3B LoRA可24GB试跑、48GB稳妥；完整3B+8B+索引推荐48GB及64–128GB主存，论文提2×48GB。费用只能据短测吞吐估，尚未得到本机或4090训练时间实测。API50元授权不自动包含租服务器。

## 2026-09-24：主基线必须够新、顶会顶刊、FRESH 查询改写

- 用户新硬条件：主 baseline 必须顶会/顶刊，优先 A 类与一区、够新、RAG query rewrite FRESH；不再因容易实现就将2023方法定为最新主基线。会议 CCF A 与期刊 CAS一区/JCR Q1 分开核验，Findings/短文/Workshop不冒称 A 类主会长文。
- 筛选报告：docs/GrowRAG_顶会FRESH基线筛选_2026-09-24.md。推荐候选 S2G-RAG（ACL2026长文，结构化缺口迭代，训练过的3B judge必要）与 DualRAG（ACL2025长文，有非FT版本，含查询生成但不是纯前置rewrite模块）。这是推荐，不是用户已确认实施；纯前置/免训练/新A/Hotpot原生全部满足的候选本轮未核实。
- S2G公开仓库未找到成品adapter；GPT开关不免除judge。现有S2G_GAP2仍是启发式适配。DualRAG当前仓库默认模型不同于论文且有无限重试/提示示例覆盖，实施须冻结实际生效配置。两仓库未发现LICENSE，不直接复制并公开作者源代码。
- MaFeRw（AAAI2025）和TTR（AAAI2026）是真实顶会训练型改写，但分别偏对话与领域IR，不强塞到当前Hotpot单题。DMQR正式录用未核实，不能声称被录用或被拒。训练free与historyfree是不同维度。
- 本轮2023 Query2doc/IRCoT阅读包未发布草稿已移除，并保留README说明；旧已导入RDF与个人批注不改。Obsidian新增筛选笔记。主代码、API费用、旧实验分数均未改变，本轮不声称任何新baseline跑通，不先租GPU。

## 2026-09-23 续轮：共享执行与真实判断检查

- 计划 knowledge/experiments/2026-09-23_判断器与共享执行_修订计划.md；结果 knowledge/experiments/2026-09-23_共享执行与判断器检查_真实结果.md。按实验研究流程先冻结条件、再调用、最后分别报告协议/语义/效果，失败不改为成功。
- 77333a4 clean启动，8次固定Qwen快照合成检查。7份类型合法、6符合全部预设检查；二选一仍要求排除另一项，属于最小证据任务口径不稳定（不是开放世界数学错误）；累计证据案例 useful_gain 返回列表而非bool/null，严格拒绝。JSON语法8/8合法不等于协议/语义可靠。
- runs/2026-09-23_judge_paired_stability_v1 保留失败与claim；本轮没有任何新Hotpot调用，也不通过换目录重跑旧失败。后缀4题已通过旧两run只读审计，仍待新的已声明协议，不是盲测题。
- paired_execution.py 只在本次同题共享完全一致Reader/judge输入；缓存不是长期经验。回放local_compute新增0用量，保留源audit；共享prefix/组件成本补回单方法影子账，不能重复算作实际账单，也不能拿缓存耗时宣称部署加速。改写和路由仍独立调用，无REUSE的差异不能解释为历史动作收益。
- 后续离线准备可选严格JSON Schema：四类组件各自固定结构，未知prompt提前拒绝，不自动降级，保留本地引用/语义验证。Schema文本计入预算预留、schema身份计入共享键。新模式未进行真实API验证，不能称问题已彻底修复。
- 最新8请求估算0.0017968元/预留0.0156270元；合计670/0.1072674/0.8392614，历史1未知继续保留。下一次核账必须额外纳入本次final_budget根，不复用只含旧三个extra roots的旧启动器。
- 下一步语义验证应成对改变题目要求/可用证据：至少一个vs唯一、BOTH缺一vs齐全、相关新年份vs无关颜色。不能只调旧8题到通过；也不能把6/8宣称一般judge准确率。
- 候选召回诊断另见 knowledge/experiments/2026-09-23_候选召回与零复用诊断.md。不要先盲目降低安全阈值；应检查是否给选择器提供了值得用的候选。三研究问题不变，先等数量强FRESH/静态规则/历史机会，再条件表示与选择器训练。
- 确定性诊断：8题同组三卡、非零词重合全来自of/and；其余3卡从不被提供。Sep23实际只有3次PRE带记忆路由，均模型直接BASE、无硬门槛拒绝REUSE。条件中有待求答案事实（如不同国家）与Hotpot桥接不合的限制，先审查可观察性，不据此私自改原卡或降低门槛。
- 本轮末1316项离线测试通过，src/tests/scripts静态及155文件格式检查通过。真实付费测试代码77333a4；其后严格Schema修补只离线测试。Obsidian新增最新实验笔记，旧阅读区不覆盖。远端状态以实际推送核验为准，不把本地提交当GitHub同步。
- 本轮GitHub同步已恢复：85e4007及之前本地提交成功推送到origin/codex/representation-selection-v1，随后ls-remote核验对应提交。未改main、未建PR、未上传密钥/原始API审计/数据；不宣称远端CI已通过。

## 2026-09-23：确认三问题、接通系统、如实保留中断

- 用户已确认：历史是否提供强FRESH没有的机会；有机会时PRE信息能否选中；有效的是适用条件还是长度/候选/检索/实体记忆。范围是固定模型和预算内的机会，不声称FRESH理论不可能。
- 当前系统入口 docs/GrowRAG_三问题与可运行系统_2026-09-23.md；controller.py、outer_loop.py、run_bounded_system.py。固定prompt的PRE三选一路由（词重合top3+显式条件）、POST无卡直接FRESH；不是已训练QPP。两层记忆保留，原6卡只有PRE，不改成POST；来源成功不等于可信。
- 真实结果 knowledge/experiments/2026-09-23_闭环真实诊断结果与下一步.md。b02f887原8题v1在000的第20次judge缺reason中止；efd3b89仅续接从未开始7题，35调用、原001/002完整、原003 judge非法JSON再次停止。原004—007未开始。新批次未重跑旧20，原日志/claim均保留。
- 六臂：BASE1/FRESH1/S2G_GAP2/REFLECTIVE2/ADAPTIVE_NO_MEMORY2/ADAPTIVE_MEMORY2。两完整题原始F1分别.3333/.3333/.8333/.8333/.8333/.8333；放行后S2G降.3333，其他不变。仅n2不得排名；记忆组0REUSE，不作增益证明。S2G用description拼接非原target/slot/训练judge，Reflective也只是同底座适配，完整ReFormeR历史基线仍未接通。
- 关键电影案例：BASE只覆盖一部；缺口拼接答对但未补证据；自由缺口查询找到另一电影年份，支持.5→1且答对。当前“原问题明确→BASE”的直觉没预测到单实体主导检索；可转化为未来可测风险，而不是宣称已学会PRE预测。
- 原000三个Reader完整请求hash相同，却空答/Delft/SouthHolland不同；T0不等于确定性。未来相同Reader输入应共享执行或预注册重复，不把漂移记到历史头上。原003需避免把普通二选一误拆成必须证明另一个不满足。
- 首次缺reason容忍本地占位（parser v2），不伪造模型理由。第二次输出885tokens且finish stop，非token上限截断；当前新增JSON object mode + judge prompt v2短输出/最小必要项。保留预算token上限与核心验证；JSON模式只管语法，不保证事实，后续修正未收费验证。本轮不再第三批。
- 预算新增55请求、估算.0157142、预留.1104316，累加历史为662/.1054706/.8236344，仍1历史未知；未来根账本必须包含09-20以及Sep23两个final_budget，不能重置。当前旧continuation提示不一致会拒绝，不能删锁换目录绕过。
- 文献同日核查：最可能回忆Useful Memories；强模型少调用对应Zhao ExpWeaver(arXiv2605.07164，仅确认预印本)。与Feng同名2606.01041(ICML2026，latentRAG)分开，旧发表标签有混淆，新增订正不覆旧包。QCR已有有无记忆差值，不能把差值当创新。
- 下阶段先稳定judge/共享执行，再机会矩阵（同数量强FRESH/静态规则/历史）；有机会才训练PRE选择器/删条件，不先上RL。原24检查、旧库保持冻结；无新建卡、无参数训练、无租卡。本轮代码先本地提交，不宣称GitHub已推送。
- 最终1220测试/静态/145文件格式检查通过；两run各有独立只读analysis目录。累计核账JSON单列662请求。Obsidian新增三笔记，共46份、双链无断链；阅读记录不改。

## 2026-09-20 战略回顾、反证优先与新预算

- 真实结果：knowledge/experiments/2026-09-20_PRE八题真实结果与选题修正.md。计划104/实际104、8题48执行臂全部完成，返回均qwen3.7-flash-2026-07-15；代码a6cb275启动clean；1074测试、静态与135文件格式检查通过。BASE EM2/8 F1.3333；FRESH4/8 .5833；RRR3/8 .4583；规则示例选择3/8、加条件4/8。即使三候选事后最佳也不超FRESH答案分，不证历史独立收益。
- 关键案例：004军事历史关键词卡误用于地理关系，规则视图选错，加条件回FRESH；001历史成功但FRESH同query；007 FRESH短No命中gold但缺Miguel国籍，BASE较长No得0属于格式/支持混淆，不能都算真实修复。历史c1补到Miguel智利国籍，支持1/3→2/3，却未被两视图选中；证据机会作为探索性记录，不能事后替换主指标制造成功。
- 000/005/006六路证据集合未变，均空答且unassessed，不归责尚未运行的judge/循环。005实体在原q，可比较静态双实体分解；000/006有桥接/条件筛选线索，可做POST验证但不宣称PRE理论不可能。
- 24候选中12与FRESH规范化query相同；实际23REUSE中11相同（1回退BASE）；48Reader只有21种题内有序证据输入。两个视图影子路径相对FRESH成本+48.62%/+74.51%，暂未证明低成本；影子费不能叠加实际采集账单。
- 本轮真实费用估算0.0128410元、预留0.1233264元；旧五run已知0.0769154/预留0.5898764，合计已知0.0897564/预留0.7132028、1历史费用未知。后续必须纳入新根账本；不删claim换目录重跑。无训练、无租卡、无自动晋升/改库、未推送本轮GitHub。

- 完整报告：docs/GrowRAG_战略回顾与选题报告_2026-09-20.md；检索边界：knowledge/literature/2026-09-20_选题回顾检索与主张边界.md。这是定向回顾不是穷尽综述，不保证无人做过；最终选题仍需用户精读确认。
- 新近邻：ProcMEM已有可观察激活/执行/结束条件及程序库更新；MemHarness已有来源/当前状态、拒绝或重构、来源状态消融和最小状态探针。其主实验有端到端任务成功率，不能借探针只测重构就说它没做终局评价。适用性/条件字段/删字段本身均不能主张首创。
- 其他直接先例：ReFormeR历史规则示例和POST选择；RAQG历史＋QPP；Selective Query Processing风险敏感配置选择；UMEM/Trainable Graph Memory增量奖励。TTR（AAAI2026）用于提醒强FRESH不只是简单prompt。详情有逐条一手来源。
- 暂定主问题：哪些历史修复仅凭检索前可观察条件就值得选，首轮证据又在哪些情况下改善判断；不是证明任何模型都不可能PRE。表示对照是归因工具，静态模式与等数量强FRESH是推翻“长期记忆必需”的关键对照。
- 推进顺序调整：原8调试题×原6PRE候选，固定top3，先选再执行BASE/canonical FRESH/RRR_KEYWORDS/全部REUSE；新V_RULE_EXAMPLE和V_PLUS_CONDITIONS只机械改变选择视图，不冒称旧M视图已生成。规范执行body相同，source q→q′可以给选择器，历史答案不进Reader。
- E0仅机会诊断：3历史最佳对1FRESH不能证明历史独立收益；后续同数量强FRESH、静态规则、历史，匹配RAG预算并单列生成费。单次缓存只是该次结果，扩大时预注册重复运行，全部视图共享重复结果。支持/不支持/样本不足都允许。
- 现有真实证据不能混写：09-06旧POST曾8题REUSE、与FRESH EM均0.75；09-06 PRE16题没有完成实际REUSE；09-11新6PRE候选尚待本轮转移诊断。不能说从未REUSE，也不能拿旧结果证明新机制。
- API累计上限由用户明确从5改为50元（含旧费用，不含Codex账户），本次子限3元、150请求、30分钟，固定qwen3.7-flash-2026-07-15，首个错误停止、无自动重试。历史未知费用保留；新入口拒绝未经复核的后续run，不静默重置预算。
- Obsidian D:/notes/GrowRAG：新增战略回顾、ProcMEM、MemHarness、Think Then Rewrite三篇导读；旧阅读记录与09-14快照不动。不需要再读全清单才允许最小实验。
- 暂不训练7B/RL、暂不租GPU、暂不接文档层/自动EMA合并。先确认H1历史有机会，H2可观察条件可选，H3延后选择质量—成本有利；失败时缩减路线，不不断换池直到阳性。

## 2026-09-19 架构定稿、协议断层与下一位模型交接（当轮仅规划）
