...
更新日期：2026-09-06
状态：用户同意持续开发，既有开发分支推送授权有效。0.4.0 接通独立 PRE 来源建卡与冻结库批量比较，598 项本地测试通过；新批次建库＋目标对照共用5元估算预算已授权，真实结果待运行后追加。自动可信选择仍待做，QPP 未接在线门控，无新参数训练。创新仍为待证假设；文档层后置。9 月 8 日文献跟进不扩权为付费实验。
