...
更新日期：2026-09-06
状态：用户确认统一外壳、不同动作正文，授权继续编码并推送 GitHub。0.2.0 已将语义改写／扩写卡片接入独立 RAG 外循环，完整 424 项测试通过；自动可信选择仍待做。此前 21/16 道 train 内小型 QPP 拟合／检查未用于在线门控。本轮无 API 或新训练；旧 78 次 API 的 FRESH/REUSE 均 6/8，不是新卡片效果。创新仍为待证假设；文档层后置。9 月 8 日文献跟进不扩权为付费实验。
