...
更新日期：2026-09-26（用户确定先接作者S2G代码；权重、评测隔离与历史路由审计）
状态：主线调整为作者S2G无历史版本先跑通，再接可关闭的历史查询动作适配器。固定作者提交5d842a6，24个Python文件语法检查、6项原始纯函数检查通过；完整S2G问答尚未运行，未找到可核验的官方Judge成品权重。自写S2G适配12题live入口已停用。本轮API0、训练0、建卡0；全仓1573项离线测试通过。累计仍723请求，已知估算0.1165764元、保守预留0.9119286元、历史1费用未知，授权50元。旧6PRE卡/24检查题与旧run不变，无新增S2G分数或记忆收益证据。
