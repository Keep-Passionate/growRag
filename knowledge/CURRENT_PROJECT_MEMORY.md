...
更新日期：2026-09-11
状态：0.6.0已实现表示选择实验底座，本地/干净导出921测试及GitHub双版本CI通过。新64来源真实运行完成：198次固定Qwen API、估算0.0295722元、6候选卡；目标题与表示排名未运行，卡片条件需复核。用户已选最少适用条件优先，本批5元总上限不跨进程重置；API已停止、不自动扩源。QPP门控、训练、文档层/多轮后置，创新待证。
