...
更新日期：2026-09-06
状态：用户要求继续编码跑测试，既有开发分支推送授权仍有效。0.3.0 新增首次检索前三分支比较与逐题报告；自动可信选择仍待做。此前 21/16 道 train 内小型 QPP 拟合／检查未用于在线门控。本轮无 API 或新训练；旧 78 次 API 的 FRESH/REUSE 均 6/8，不是新卡片效果。创新仍为待证假设；文档层后置。9 月 8 日文献跟进不扩权为付费实验。
