...
2026-09-06 最新：0.3.0 接通首次检索前的 BASE／FRESH／REUSE 同题比较，固定改写目的、共享 prompt，输出逐题答案／证据／费用与失败报告。完整 497 项本地测试通过。先看 [三分支实验与代码连接图](docs/query_comparison_v1.md) 和 [版本记录](CHANGELOG.md)。本轮只做本地测试和零 API 演示；自动可信选择与新协议真实模型评价仍未完成，不能把流程测试当模型效果。
