...
最新 0.6.0 开发：接通[表示选择实验底座](docs/representation_selection_v1.md)，四种经验记法共用候选和规范动作结果。后续已选择[最少适用条件路线](knowledge/decisions/2026-09-11_条件最小化路线与分阶段实施.md)：先逐项删条件，查误用和误拒；QPP 后置。本批真实 API 总上限 5 元，先来源可行性检查；目标表示实验尚未完成，不预报正向效果。

本轮已完成[64 来源真实诊断](knowledge/experiments/2026-09-11_来源64题真实结果与条件风险.md)：198 次固定 Qwen API，估算 0.0295722 元，生成 6 张候选卡。卡中已发现“需先知道答案才可检查”的错误适用条件，尚不能称可信经验；没有运行目标题。921 项本地/干净副本测试及 GitHub Python 3.11/3.12 CI 通过，代码已推送到独立 `codex/representation-selection-v1` 分支，main 未改。
