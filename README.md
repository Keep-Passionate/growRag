...
2026-09-06 最新：0.2.0 实现“统一卡外壳＋语义改写／扩写正文”，通过多来源只读视图接入外置 RAG 循环；424 项本地测试通过。先看 [代码地图与使用边界](docs/query_cards_v1.md) 和 [版本记录](CHANGELOG.md)。当前经验仍由实验显式指定，自动可信选择与新循环真实模型评价未完成，不能把流程测试当模型效果。
...
`src/growrag` 是 2026-08-19 新建的研究脚手架。旧版 GrowRAG 已退出活动工程并归档。原有代码保留历史复用子模块及 QueryEpisode/ExperienceCard 数据合同。09-06 的 `experiments/` 保持单步配对执行器；最新 `outer_loop.py` 提供外部有限循环，`query_actions.py` 分离形式／目的／来源，`preflight_qpp.py` 提供独立本地预测诊断。新循环尚无自动经验选择或真实充分性 judge，不能称为已完成的可信 Agentic RAG。
