...
最新进展（2026-09-23）：接通[查询侧路由＋有界修复闭环](docs/GrowRAG_三问题与可运行系统_2026-09-23.md)。确认三条研究问题：历史额外机会、检索前能否选中、适用条件独立作用。当前累计API预算50元，完整权威状态见[项目记忆](knowledge/CURRENT_PROJECT_MEMORY.md)。

最新续轮：接通同题Reader/judge共享和两套成本账。8次真实合成检查中，6符合预定标准、1任务口径不一致、1字段类型错误，按计划停止，4道Hotpot后缀未启动。见[共享执行与判断器检查](knowledge/experiments/2026-09-23_共享执行与判断器检查_真实结果.md)。严格字段约束后续仅离线准备，尚无记忆收益证据。

上一轮55次真实API，2题完成六组对照，另2题因judge输出协议错误中止，4题未开始；见[闭环真实诊断](knowledge/experiments/2026-09-23_闭环真实诊断结果与下一步.md)。已观察到无历史缺口修复补证据的案例，不能将其归因于长期记忆。

历史0.6.0：接通[表示选择实验底座](docs/representation_selection_v1.md)，四种经验记法共用候选和规范动作。[64来源诊断](knowledge/experiments/2026-09-11_来源64题真实结果与条件风险.md)形成6候选卡，至今未自动晋升；“最少适用条件”仍保留，但先确认机会是否存在。

研究构思入口：[从外置查询层到可信经验复用](docs/GrowRAG_项目构思与文献脉络_2026-09-07.md)，包含整体闭环图、REUSE−FRESH 配对图、模块设计、文献脉络和实现边界；[全部历史文献索引](docs/GrowRAG_构思文档_历史文献索引_2026-09-07.md)可按模块查阅。该文是研究规划，不是已取得记忆增益的声明。

上一版0.5.0：增加[简明逻辑图与候选匹配原型](docs/pattern_matching_v1.md)。先识别明确问题模式，再核对卡片来源、版本和显式范围；匹配结果只是CANDIDATE，不自动可信复用。实际做了零API诊断：9个合成边界题和旧16个开发题分别报告，未新增答案成绩。

GrowRAG 已确认方案 B「可信自适应查询修复」。当前目标是在保留基础检索器、索引与回答模型的前提下，建立一个可绕过、可追溯、可拒绝历史经验的轻量修复闭环；控制层会改变查询与调用流程，并非整个执行架构完全不变：

> 前置查询层选择原查询或改写，进入独立 RAG；结果返回外层后，决定停止或继续。改写既可改善语义表达，也可针对已见证据缺口；原始问题始终保留。
...
2026-09-06／07 最新开发：0.4.0 接通独立 PRE 来源建卡与冻结库批量比较，真实完成32来源／16内部目标：生成1卡、16题均无适用卡，BASE 6/16、FRESH 5/16，**REUSE未执行，尚不能评价记忆收益**。145次API估算0.0209324元；随后仅离线排查，不自动追加收费批次。先看 [首批真实结果与下一步](knowledge/experiments/2026-09-06_PRE真实建库与比较_结果.md) 和 [实现说明](docs/pre_pilot_v1.md)。候选选择仍不是可信门控。

历史0.4.1补上无卡／失败／弃答分类和免费检索消融工具，633项本地测试通过。来源诊断发现“仅实体名”与完整成功改写检出同序证据，尚未证明卡片概括的具体操作就是增益原因；未据此晋升卡或追加API。

前一版 0.3.0 接通首次检索前的 BASE／FRESH／REUSE 同题比较；当时497项测试通过，只有零API替身演示。[三分支实验与代码连接图](docs/query_comparison_v1.md) 和 [版本记录](CHANGELOG.md) 保留。

前一版 0.2.0 实现“统一卡外壳＋语义改写／扩写正文”，通过多来源只读视图接入外置 RAG 循环；当时 424 项本地测试通过。[经验卡代码地图](docs/query_cards_v1.md) 仍可用于理解卡片结构。

上一轮：已增加最小外置循环、统一动作接口和经验改版重新验证边界，当时 382 项本地测试通过。QPP 在 21 道内部训练题拟合、16 道内部开发题检查，尚未用于路由；见 [外循环与本地训练记录](knowledge/decisions/2026-09-06_外置查询层_动作分流与最小实现.md)。

此前单步试跑：已接通 train 数据划分、本地句级 BM25、固定 prompt 的 gap、真实模型三分支、来源经验提取与冻结、逐题分层反馈和费用审计。**这是小型研究执行器，不是已证明有效的可信控制器。** 首轮冻结 8 个来源题＋8 个开发题；真实结果如下，原 mock 演示不能当研究结果。文档层后置，Token Plan 个人版不用于实验脚本。

此前真实试跑已完成：78 次 API，估算 0.0148676 元；8 道目标题 BASE 答对 3 题、FRESH 与 REUSE 均答对 6 题，**没有观察到记忆优于 FRESH**。生成 2 条候选经验；逐题发现了引用和答案不一致等问题。详见 [真实结果与下一步](knowledge/experiments/2026-09-06_首轮真实Hotpot试跑_结果与下一步.md)。当时 336 项测试通过，该批代码在 [旧试跑分支](https://github.com/Keep-Passionate/growRag/tree/codex/hotpot-pilot-v1)；新外循环及卡片版本使用 [独立开发分支](https://github.com/Keep-Passionate/growRag/tree/codex/query-card-actions-v1)，不自动合并 main。
...
- [2026-09-23续轮：共享执行、判断器边界、停止原因与下一步](knowledge/experiments/2026-09-23_共享执行与判断器检查_真实结果.md)
- [2026-09-23系统实现：路由、FRESH、记忆结构和三问题](docs/GrowRAG_三问题与可运行系统_2026-09-23.md)
- [2026-09-23真实诊断与后续执行边界](knowledge/experiments/2026-09-23_闭环真实诊断结果与下一步.md)
- [表示选择实验：代码与当前边界](docs/representation_selection_v1.md)
- [已选最少适用条件：研究问题、消融与阶段计划](knowledge/decisions/2026-09-11_条件最小化路线与分阶段实施.md)

- [下一步、简明逻辑图与候选匹配代码](docs/pattern_matching_v1.md)
- [PRE 首批真实结果：覆盖不足与查询成分的竞争解释](knowledge/experiments/2026-09-06_PRE真实建库与比较_结果.md)
- [正式开发首批：独立 PRE 来源、冻库与真实比较入口](docs/pre_pilot_v1.md)
- [前置三分支比较：当前实现、逐题报告与边界](docs/query_comparison_v1.md)
- [统一经验卡 v1：当前代码地图与运行边界](docs/query_cards_v1.md)
- [前置查询层＋RAG 外循环：代码图、动作分流与本地训练](knowledge/decisions/2026-09-06_外置查询层_动作分流与最小实现.md)
...
`src/growrag` 是 2026-08-19 新建的研究脚手架。旧版 GrowRAG 已退出活动工程并归档。原有代码保留历史复用子模块及 QueryEpisode/ExperienceCard 数据合同。`experiments/paired_runner.py` 保留旧单步协议；新 `query_comparison.py` 在首次检索前从空证据比较三分支。`outer_loop.py` 提供外部有限循环，`query_actions.py` 分离形式／目的／来源，`preflight_qpp.py` 提供独立本地预测诊断。新循环尚无自动经验选择或真实充分性 judge，不能称为已完成的可信 Agentic RAG。
