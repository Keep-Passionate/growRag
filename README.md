...
最新0.5.0：增加[简明逻辑图与候选匹配原型](docs/pattern_matching_v1.md)。先识别明确问题模式，再核对卡片来源、版本和显式范围；匹配结果只是CANDIDATE，不自动可信复用。实际做了零API诊断：9个合成边界题和旧16个开发题分别报告，未新增答案成绩。
...
当前0.4.1补上无卡／失败／弃答分类和免费检索消融工具，633项本地测试通过。来源诊断发现“仅实体名”与完整成功改写检出同序证据，尚未证明卡片概括的具体操作就是增益原因；未据此晋升卡或追加API。
